	...
	public function render_item( $term, $current_term, $layout ) {
		...
		if ( $this->get_instance_value( 'show_image' ) === 'yes' ) {
			...
			if ( $this->get_instance_value( 'image_type' ) === 'custom_field' ) {
				...
				$img = get_term_meta( $term->term_id, $this->get_instance_value( 'ae_taxonomy_bg_cf_field_key' ), true );
				if ( $img ) {
					//$image = $img[0];
					$image = $img;
				} else {
			...
			$alt_text_type = $this->get_instance_value( 'image_alt_text' );
			$alt_text = $this->get_image_alt_text($alt_text_type, $term);
			if($alt_text !== ''){
				$image = wp_get_attachment_image( $image, $img_size, false, array('alt' => $alt_text));
			}else{
				$image = wp_get_attachment_image( $image, $img_size);
			}
