	...
	public function get_multiple_name_control() {
		...
		$controls        = [
			...
			[
				...
				'object_type' => '',
				'condition'   => [
					'ae_rule_acf_relationship_parent_type_name' => 'none',
				],
			],
			//repeater field
			[
				...
				'condition'   => [
					'ae_rule_acf_relationship_parent_type_name' => 'repeater',
				],
			],
			//Group field
			[
				...
				'condition'   => [
					'ae_rule_acf_relationship_parent_type_name' => 'group',
				],
			],
			[
				'label'        => __( 'Sub Field', 'ae-pro' ),
				'type'         => 'aep-query',
				'parent_field' => 'ae_rule_acf_relationship_acf_repeater_field_name',
			...
			[
				'label'           => __( 'Sub Field', 'ae-pro' ),
				'type'            => 'aep-query',
				'parent_field'    => 'ae_rule_acf_relationship_acf_group_field_name',
		...
		switch ( $parent ) {
			case 'repeater':
				...
				if ( Frontend::$_in_repeater_block == true ) {
					...
					if ( ! empty( $sub_field_obj ) ) {
						...
						$selected_posts = $sub_field_obj['value'];
			...
			case 'group':
				...
				if ( isset( $field_data[0] ) && $field_data[0] == 'option' ) {
					...
					$return_format  = $sub_field_obj['return_format'];
					$selected_posts = $grp_field_value[ $name['sub_field'] ];
					if ( empty( $selected_posts ) ) {
						return;
					}
					if ( ! empty( $selected_posts ) && 'object' === $return_format ) {
			...
			case 'none':
				...
				if ( isset( $field_data[0] ) && $field_data[0] == 'options' ) {
					...
					$selected_posts = $field_object['value'];
					...
					$field_data     = get_field( $name, $data );
					$selected_posts = $field_object['value'];
					if ( ! empty( $field_object ) ) {
						$return_format = $field_object['return_format'];
						if ( ! empty( $selected_posts ) && 'object' === $return_format ) {
